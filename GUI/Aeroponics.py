...
def run_aeroponics():
    ...
    serAe = serial.Serial('COM3', 9600, timeout=1)  
    time.sleep(2) 
    ...
    fig.suptitle('Aeroponics', fontsize=16)
    ...
    def read_serial(frame):
        try:
            ...
            data = serAe.readline().decode('utf-8').strip()  
            ...
            if CO2:
                avg_CO2 = np.mean(CO2)
                axs[0, 1].plot(CO2,'o-', label="CO2", linewidth=2, marker='o', markersize=4)
                axs[0, 1].text(0.5, 1.05, f"Avg: {avg_CO2:.2f}", ha='center', va='center', transform=axs[1, 0].transAxes, fontsize=10)
                axs[0, 1].set_title("CO2 Sensor", pad=20)
                axs[0, 1].legend()
                axs[0, 1].grid(True)

            if Temp:
                avg_Temp = np.mean(Temp)
                axs[1, 0].plot(Temp, 'b-',label="Temp", linewidth=2, marker='o', markersize=4)
                axs[1, 0].text(0.5, 1.05, f"Avg: {avg_Temp:.2f}", ha='center', va='center', transform=axs[1, 1].transAxes, fontsize=10)
            ...
            if Humidity:
                ...
                axs[1, 1].plot(Humidity, 'c-', label="Humidity", linewidth=2, marker='o', markersize=4)
                axs[1, 1].text(0.5, 1.05, f"Avg: {avg_hum:.2f}",ha='center', va='center', transform=axs[2, 0].transAxes, fontsize=10)
                axs[1, 1].set_title("Humidity Sensor", pad=20)
                axs[1, 1].legend()
                axs[1, 1].grid(True)

            if H2OPO4:
                avg_h2opo4 = np.mean(H2OPO4)
                axs[2, 0].plot(H2OPO4, label="Water PH", linewidth=2, marker='o', markersize=4)
                axs[2, 0].text(0.5, 1.05, f"Avg: {avg_h2opo4:.2f}", ha='center', va='center', transform=axs[2, 1].transAxes, fontsize=10)
                axs[2, 0].set_title("Water PH Sensor", pad=20)
                axs[2, 0].legend()
                axs[2, 0].grid(True)
